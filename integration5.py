...
from sklearn.metrics import accuracy_score, recall_score  # NEW: metrics
...
MODEL_METRICS_JSON = "model_metrics.json"  # NEW: metrics file
...
def compute_and_save_metrics(dataset, device, gnn_model, lstm_model, xgb_algo, xgb_arch, xgb_proto, ocsvm):
    """
    Compute accuracy, recall, and OC-SVM false positive / false negative rates
    on the synthetic dataset and save to MODEL_METRICS_JSON.
    """
    ...
    lstm_acc = float(accuracy_score(labels, preds))
    lstm_recall = float(recall_score(labels, preds, average="macro"))
