...
#    (you can change these to match your real dataset)
# =========================================================

# 20+ crypto primitives
ALGO_ID_TO_NAME = {
    0: "AES-CBC",
    1: "AES-CTR",
    2: "AES-GCM",
...
# file that stores protocol sequences learned / defined in dataset
...
#    (for training and inference; currently byte-based demo)
# =========================================================
...
def build_features_from_bytes(data: bytes):
    """
    Shared feature extractor used for:
      - training synthetic dataset
      - inference on real binary
    Returns:
      gnn_data: PyG Data
      lstm_input: (seq[1,L], lengths[1], extra_feats[1,F])
      tabular: np.array shape (1, D)
      feature_dict: human-readable values
    """
    if len(data) == 0:
        data = b"\x00"
    ...
    seq = torch.tensor(opcode_ids, dtype=torch.long).unsqueeze(0)  # [1, L]
    lengths = torch.tensor([len(opcode_ids)], dtype=torch.long)   # [1]
    ...
    # ---- Tabular features (XGBoost / OC-SVM) ----
...
#    (replace later with your real dataset)
# =========================================================

def synthetic_sample(algo_id: int):
    """
    For demo training: generate synthetic bytes whose
    distribution depends on algo_id.
    """
    random.seed()
    base_len = 500 + 50 * algo_id
    length = random.randint(base_len, base_len + 200)

    # choose center byte for this algo_id
...
def build_synthetic_dataset(n_samples=400):
    """
    Build synthetic dataset:
      - each sample has algo_id, arch_id, proto_id
      - we generate bytes, extract features using the same pipeline
      - we also store per-algorithm average feature vector
    """
    ...
    # for per-algo mean features
    per_algo_features = {aid: [] for aid in ALGO_ID_TO_NAME}

    # simple synthetic protocol sequences mapping (will be saved as dataset)
    proto_sequences = {}

    for proto_id, proto_name in PROTOCOL_ID_TO_NAME.items():
        # toy sequence data (you would replace with real dataset later)
    ...
    # compute per-algorithm mean features
    algo_feature_means = {}
    ...
    # save protocol sequences dataset
...
def train_all_models():
    ...
    dataset = build_synthetic_dataset(n_samples=500)
    ...
    gnn_model = GNNClassifier(gnn_in_dim, hidden_dim=64, num_classes=gnn_num_classes).to(device)
    ...
    # ---------- XGBoost (algorithm + architecture + protocol) ----------
    ...
    # ---------- OC-SVM ----------
    # treat first half of algorithms as "standard"
...
def load_all_models():
    ...
    # GNN
    gnn_in_dim = 2
    gnn_model = GNNClassifier(gnn_in_dim, 64, len(ALGO_ID_TO_NAME)).to(device)
...
def predict_for_binary(bin_path: str):
    ...
    # ---- OC-SVM standard vs proprietary ----
    oc_pred = int(ocsvm.predict(tabular)[0])   # +1 = standard, -1 = proprietary
    is_proprietary = (oc_pred == -1)

    # ---- majority vote for final algorithm ----
    ...
    # architecture / protocol from dedicated XGB models
    ...
    # protocol sequence from dataset (NOT hard-coded)
